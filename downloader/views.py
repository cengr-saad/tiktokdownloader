...
def home_download(request):
    return render(request, 'home.html')

def download(request):
    video_url = request.GET['video_url']
    video = downloader(video_url)
    #return video
    #url = downloader(video_url)
    #return redirect(video)
    return JsonResponse({'video_url': video_url})

def downloader(url):
    api = Scraper()
    #url = 'https://www.tiktok.com/@cznburak/video/7267946224807054598?is_from_webapp=1&sender_device=pc'  # Replace with your video URL
    data = asyncio.run(api.hybrid_parsing(url))

    if data['status'] == 'success' and data['type'] == 'video':
        url = data['video_data']['nwm_video_url_HQ']
        #save_path = os.path.join(settings.MEDIA_ROOT, timezone.now().strftime('%Y_%m_%d_%H_%M_%S.mp4'))
        #save_path = 'E:/Programs/Tiktok_Downloader/videos/' + file_name  # Replace with your desired file path
        #response_video = download_video(url, save_path)
        #response_video = download_video(url)
    else:
        print('Failed to retrieve video data or the URL is not a video.')
    return url
...
'''def download_video(video_url):
    ...
    if response.status_code == 200:
        ...
        def data_iterator(video_data, chunk_size):
            offset = 0
        ...
        response_data = StreamingHttpResponse(data_iterator(data, 8192))
        response_data['Content-Disposition'] = 'attachment; filename="video.mp4"'
        response_data['Content-Type'] = 'video/mp4'
        #response_data = StreamingHttpResponse(data, content_type='video/mp4')
        #response_data['Content-Disposition'] = 'attachment;filename="video.mp4"'
...
class Scraper:
    ...
    async def hybrid_parsing(self, video_url: str) -> dict:
        # URL Platform judgment/Judge URL platform
        url_platform = 'douyin' if 'douyin' in video_url else 'tiktok'
        print('The current link platform is: {}'.format(url_platform))
        ...
        video_id = await self.get_douyin_video_id(
            ...
            video_url)
        if video_id:
            print("Obtaining the video ID successfully, the video ID is: {}".format(video_id))
            # Get video data /Get video data
            print("Obtaining video data...")
            data = await self.get_douyin_video_data(
                video_id) if url_platform == 'douyin' else await self.get_tiktok_video_data(
                video_id)
            if data:
                print("Video data obtained successfully, data type is being determined...")
                url_type_code = data['aweme_type']
                """The following is the Douyin/TikTok type code/Type code for Douyin/TikTok"""
                url_type_code_dict = {
                    # 抖音/Douyin
                    2: 'image',
                    4: 'video',
